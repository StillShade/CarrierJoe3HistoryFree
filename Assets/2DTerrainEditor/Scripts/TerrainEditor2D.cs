...
public class TerrainEditor2D : MonoBehaviour
{
    ...
    /// <summary>
    /// Terrain width in units
    /// </summary>
    public int Width
    {
        get { return _width; }
        set { _width = Mathf.Clamp(value, 10, 10000); }
    }
    ...
    {
        get { return _height; }
        set { _height = Mathf.Clamp(value, 10, 10000); }
    }
    private int _height = 250;

    /// <summary>
    /// Terrain segments per unit
    /// </summary>
    public int Resolution
    {
        get { return _resolution; }
        set { _resolution = Mathf.Clamp(value, 1, 50); }
    }
    ...
    /// <summary>
    /// Terrain mesh color. Works only if shader supports vertex colors
    /// </summary>
    ...
    #region Random generation values
    /// <summary>
    /// Average height for random terrain generation
    /// </summary>
    public float RndHeight
    {
        get { return Mathf.Clamp(_rndHeight, 0.1f, Height); }
        set { _rndHeight = value; }
    }
    private float _rndHeight = 25;

    /// <summary>
    /// Max hills count for random terrain generation
    /// </summary>
    public int RndHillsCount
    {
        get { return Mathf.Clamp(_rndHillsCount, 1, Width); }
        set { _rndHillsCount = value; }
    }
    private int _rndHillsCount = 5;

    /// <summary>
    /// Average amplitude for random terrain generation
    /// </summary>
    public float RndAmplitude
    {
        get { return Mathf.Clamp(_rndAmplitude, 0.1f, Height); }
        set { _rndAmplitude = value; }
    }
    private float _rndAmplitude = 5;
    #endregion

    #region Cap settings values
    /// <summary>
    /// Is cap needs to be generated for this terrain?
    /// </summary>
    public bool GenerateCap;

    /// <summary>
    /// Cap material of this terrain with tiled texture
    /// </summary>
    public Material CapMaterial;

    /// <summary>
    /// Number of cap texture tiles for whole tterrain (does not affect to cap with <see cref="SmartCap"/> enabled)
    /// </summary>
    ...
    /// <summary>
    /// Is this terrain could be deformed in runtime?
    /// </summary>
    ...
    /// <summary>
    /// Is side end points of terrain path needs to be fixed and protected from changes? 
    /// </summary>
    ...
    /// </summary>
    public float LeftFixedPoint
    {
        get { return Mathf.Clamp(_leftFixPoint, 0.1f, Height); }
        set { _leftFixPoint = value; }
    }
    private float _leftFixPoint = 25;

    /// <summary>
    /// This value will be changed after applying random terrain generation. WARNING: Value can not be more than <see cref="Height"/>
    /// </summary>
    public float RightFixedPoint
    {
        get { return Mathf.Clamp(_rightFixedPoint, 0.1f, Height); }
        set { _rightFixedPoint = value; }
    }
    private float _rightFixedPoint = 25;
    /// <summary>
    ...
    /// <summary>
    /// Returns all vertices of this terrain mesh
    /// </summary>
    ...
    /// <summary>
    /// Returns all vertices of cap mesh
    /// </summary>
    /// <returns>Array of all vertices points</returns>
    public Vector3[] GetVertsPosCap()
    {
        if (!GenerateCap || CapObj == null || CapObj.GetComponent<MeshFilter>() == null || CapObj.GetComponent<MeshFilter>().sharedMesh == null)
    ...
    /// <summary>
    /// Sets mesh ID for this terrain (Editor only)
    /// </summary>
    ...
    /// <summary>
    /// Fully updates terrain based on given vertices array (Editor only)
    /// </summary>
    ...
    /// <summary>
    /// Creates new 2D terrain
    /// </summary>
    /// <param name="position">Position in world space</param>
    /// <returns>TerrainEditor2D object</returns>
    public static TerrainEditor2D CreateNew(Vector3 position)
    {
        GameObject tObj = new GameObject("New Terrain 2D");
        tObj.transform.position = position;

        TerrainEditor2D t = tObj.AddComponent<TerrainEditor2D>();
        t.Create();
    ...
    /// <returns>TerrainEditor2D object</returns>
    public static TerrainEditor2D CreateNew(Vector3 position, int width, int height, int resolution)
    {
        GameObject tObj = new GameObject("New Terrain 2D");
        TerrainEditor2D t = tObj.AddComponent<TerrainEditor2D>();
